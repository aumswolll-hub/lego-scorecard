...
- script = โครงคลิป 30-60 วินาที เขียนเป็นบีท 5-7 บีท ไล่ตามอารมณ์:
  เปิดด้วย pain → ขยี้ให้เห็นภาพ/relate → ยกระดับความเจ็บ (สิ่งที่เสียถ้าปล่อยไว้) → พลิกเข้าสินค้า → สาธิต/เหตุผลว่าทำไมต้องตัวนี้ → กันข้อสงสัย 1 อย่างสั้นๆ → ปิดด้วย CTA
- ทุกบีทต้องมี "คำพูดตัวอย่าง" ที่พูดได้จริงหน้ากล้อง (ไม่ใช่แค่บอกว่าทำอะไร) + วงเล็บบอกภาพที่ควรเห็น เช่น บีท 1: "หวีทีนึงผมติดมาเป็นกำ..." (โชว์หวีที่มีผมติด)
...
{"angles":[{"name":"ชื่อมุมสั้นๆ","angle_type":"ประเภทมุมจากคลัง","pain":"pain point ที่มุมนี้ตี (1 ประโยค)","hook":"ประโยคเปิดคลิป","script":"บีท 1: \\"คำพูดตัวอย่าง...\\" (ภาพที่ควรเห็น) / บีท 2: ... ไล่จนครบ 5-7 บีท"}]}`;
  ...
  try {
    ...
    const aiRes = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        // script ไทย 5-7 บีทพร้อมคำพูดตัวอย่าง × 3 มุม — เผื่อ token เยอะๆ
        // (บทเรียน: 1200 เคยตัด JSON กลางทางมาแล้ว)
        max_tokens: 5000,
        temperature: 0.7,
    ...
    try {
      ...
      angles = (parsed.angles || []).slice(0, 3).map((a) => ({
        ...
        script: String(a.script || "").slice(0, 2000),
      })).filter((a) => a.hook);
    } catch (e) {
